...
def preprocessing_df(df):
    ...
    for i in range(len(df)):
        ...
        temp_str = re.sub('[-=+#/\?:^$@*\"※~&%ㆍ!』\\‘|\(\)\[\]\<\>`\'…》⭐️⭕️]', '%',df["text"][i])  #, . 을 제외한 나머지 특수문자 count를 위해 모두 %로 변경
        df["special_char_count"][i] = temp_str.count('%')     #특수 문자 개수 column 추가
        df["manufacturer_count"][i] = df["text"][i].count(df["manufacturer"][i])     #제조사 언급 횟수 column 추가
        df["product_count"][i] = df["text"][i].count(df["product"][i])     #제품명 언급 횟수 column 추가
